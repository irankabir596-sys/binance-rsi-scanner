...
def _market_text(market):
    return normalize_text(" ".join(
        str(market.get(k, ""))
        for k in ("question", "title", "description", "slug", "ticker", "eventSlug")
    ))


def market_timeframe(market):
    text = _market_text(market)
    # Check longer units first. Avoid generic "5m" substring matches in IDs.
    patterns = {
        "5m": ("5 minute", "5 min", "5-minute", "5-minute"),
        "15m": ("15 minute", "15 min", "15-minute"),
        "1h": ("1 hour", "1 hr", "1-hour", "1 hour"),
        "4h": ("4 hour", "4 hr", "4-hour"),
        "1D": ("daily", "1 day", "24 hour", "1-day"),
    }
    for tf, needles in patterns.items():
        if any(n in text for n in needles):
            return tf

    # Polymarket short-duration markets often encode the window only through
    # startDate/endDate (for example 5-minute Up/Down markets). Use duration
    # as a second, safer detector.
    def parse_dt(value):
        if not value:
            return None
        try:
    ...
    start = parse_dt(market.get("startDate") or market.get("startDateIso") or market.get("start_date_iso"))
    end = parse_dt(market.get("endDate") or market.get("endDateIso") or market.get("end_date_iso"))
    if start and end:
        minutes = (end - start).total_seconds() / 60.0
        if 4.0 <= minutes <= 6.5:
            return "5m"
        if 13.0 <= minutes <= 17.0:
            return "15m"
        if 50.0 <= minutes <= 70.0:
            return "1h"
        if 220.0 <= minutes <= 260.0:
            return "4h"
...
def _is_up_down_market(market):
    outcomes = _json_list(market.get("outcomes"))
    if not outcomes:
        return False
    names = {normalize_text(x) for x in outcomes}
    return "up" in names and "down" in names


def _market_timestamp(market):
    for key in ("endDate", "end_date_iso", "endDateIso", "closeTime"):
        value = market.get(key)
        if value:
            return str(value)
    return ""
...
def _get_polymarket_markets():
    """Fetch active markets in pages so short-term markets are not missed by limit=500."""
    markets = []
    seen = set()

    # Gamma can expose markets directly. Pagination is important because the
    # first 500 active markets are not guaranteed to contain BTC short-term markets.
    for offset in range(0, 3000, 500):
        try:
            ...
            if response.status_code != 200:
                print(f"Polymarket markets HTTP {response.status_code}")
                break
            data = response.json()
            ...
            print(f"Polymarket page error: {e}")
            break
    return markets
...
def _clob_midpoint(token_id):
    try:
        ...
        print(f"Polymarket midpoint error: {e}")
        return None


def _extract_up_down_tokens(market):
    outcomes = _json_list(market.get("outcomes"))
...
def discover_polymarket_probability(symbol, timeframe):
    """Return REAL Up/Down odds for exactly this symbol/timeframe, or None.

    The bot only accepts explicit Up/Down markets. It does not treat a generic
    Yes/No market as an Up/Down probability. Prices are read from Polymarket's
    CLOB midpoint endpoint using the market's token IDs.
    """
    base = extract_base_symbol(symbol)

    try:
        markets = _get_polymarket_markets()
        candidates = []

        for market in markets:
            if not market_mentions_asset(market, base):
                continue
            if market_timeframe(market) != timeframe:
                continue
            if not _is_up_down_market(market):
                continue

            up_token, down_token = _extract_up_down_tokens(market)
            if not up_token or not down_token:
                continue

            # Prefer explicit Up/Down markets whose question contains the asset
            # and timeframe. Score exactness so unrelated descriptions are less
            # likely to win when multiple markets exist.
            text = _market_text(market)
            score = 0
            if base.lower() in text:
                score += 10
            if timeframe == market_timeframe(market):
                score += 10
            if "up or down" in text or "up down" in text:
                score += 5
            candidates.append((score, market, up_token, down_token))

        candidates.sort(key=lambda x: x[0], reverse=True)

        for _, market, up_token, down_token in candidates:
            up = _clob_midpoint(up_token)
            down = _clob_midpoint(down_token)

            if up is None and down is None:
                # Fallback only for an explicit Up/Down market.
                outcomes = _json_list(market.get("outcomes"))
                ...
                if outcomes and prices and len(outcomes) == len(prices):
                    for outcome, price in zip(outcomes, prices):
                        try:
                            ...
                            continue
                        if normalize_text(outcome) == "up":
                            up = value
                        elif normalize_text(outcome) == "down":
                            down = value
            ...
            up = max(0.0, min(100.0, float(up) * (1.0 if float(up) > 1.0 else 100.0)))
            down = max(0.0, min(100.0, float(down) * (1.0 if float(down) > 1.0 else 100.0)))
        ...
        print(f"No active Polymarket Up/Down market: {symbol} {timeframe}")
        return None
...
def build_message(symbol, timeframe, rsi, volume_ratio, volume_emoji, trend, trend_emoji, market_probability, polymarket_odds=None, technical=None, next_candle=None, consensus=None):
    ...
    # Show ONLY the Polymarket odds for the exact timeframe of this alert.
    pm = polymarket_odds or {}
    if pm.get("up") is not None and pm.get("down") is not None:
        polymarket_block = (
...
def main():
    ...
    for signal in pending:
        polymarket_odds = discover_polymarket_probability(
            signal["symbol"],
            signal["timeframe"],
        )
        probability = polymarket_odds.get("up") if polymarket_odds else None
