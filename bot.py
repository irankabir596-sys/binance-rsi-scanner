...
def nearest_level(
    ...
    """Select the most relevant nearby S/R zone in the RSI direction.

    RSI > 80 -> resistance only.
    RSI < 20 -> support only.

    The selected zone balances proximity with structural importance:
    repeated touches, recency, and rejection strength are considered.
    """
    if idx < 20 or target_side not in {"Resistance", "Support"}:
    ...
    if target_side == "Resistance":
        raw_points = [(p, candles[p][2]) for p in highs if candles[p][2] > price]
    else:
        raw_points = [(p, candles[p][3]) for p in lows if candles[p][3] < price]

    clusters = cluster_levels(raw_points, tolerance, max_points=40)
    if not clusters:
        return None, None, None

    # Evaluate each zone. Proximity matters, but a very weak isolated level
    # should not automatically beat a nearby level with repeated reactions.
    best: Optional[Tuple[float, float]] = None
    best_level: Optional[float] = None
    for level, touches, cluster_score in clusters:
        distance = abs(level - price)
        distance_atr = distance / max(atr, 1e-12)
        if distance_atr > 4.0 and distance > price * 0.025:
            continue

        # Count historical reactions around the candidate zone.
        reactions = 0
        ...
        for j in range(start, idx):
            ...
            o = candles[j][1]
            c = candles[j][4]
            near = max(atr * 0.20, price * 0.0015)
            if target_side == "Resistance":
                if abs(h - level) <= near or (l <= level <= h and c < level):
                    reactions += 1
                    rejection_strength += max(0.0, (h - c) / max(atr, 1e-12))
            else:
                if abs(l - level) <= near or (l <= level <= h and c > level):
                    reactions += 1
                    rejection_strength += max(0.0, (c - l) / max(atr, 1e-12))

        recency = max(0.0, 1.0 - min(1.0, (idx - max(p for p, _ in raw_points if abs(_ - level) <= tolerance)) / max(1, LOOKBACK))) if raw_points else 0.0
        # Score is intentionally not a probability. It ranks candidate zones.
        importance = (
            1.25 * min(touches, 6)
            + 0.70 * min(reactions, 8)
            + 0.55 * min(rejection_strength, 6.0)
            + 1.10 * recency
        )
        proximity = 1.0 / (1.0 + distance_atr)
        score = importance * 0.72 + proximity * 0.28
    ...
    if best_level is None:
        return None, None, None
    return target_side, best_level, atr
...
def signal_for(
    ...
    target_side = "Resistance" if rsi > 80.0 else "Support"
    side, level, atr = nearest_level(candles, idx, target_side)
...
def sort_signals(signals: List[Dict[str, Any]]) -> None:
    # Highest RSI first; timeframe is the secondary sort key.
    signals.sort(
        key=lambda s: (
            -float(s["rsi"]),
            TF_ORDER.get(s["tf"], 99),
            s["symbol"],
        )
    )
...
def send_signal_messages(signals: List[Dict[str, Any]], chat_ids: Sequence[str]) -> None:
    ...
    # One Telegram message per coin/timeframe, in RSI descending order.
    for cid in chat_ids:
        for sig in signals:
            send_message(cid, render_signal(sig))
