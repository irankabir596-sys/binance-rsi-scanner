...
def discover_polymarket_odds(symbol):
    """Return live Polymarket UP/DOWN probabilities for all supported timeframes."""
    base = extract_base_symbol(symbol)
    odds = {tf: None for tf in TIMEFRAMES}
    try:
        response = session.get(POLYMARKET_URL, params={"active":"true","closed":"false","limit":500}, timeout=REQUEST_TIMEOUT)
        if response.status_code != 200:
            print(f"Polymarket HTTP {response.status_code}: {response.text[:300]}")
            return odds
        data = response.json()
        markets = data.get("data") or data.get("markets") or [] if isinstance(data, dict) else data if isinstance(data, list) else []
        for market in markets:
            if not isinstance(market, dict) or not market_mentions_asset(market, base):
                continue
            tf = market_timeframe(market)
            if tf not in odds or odds[tf] is not None:
                continue
            p = parse_outcome_probability(market)
            if p is not None:
                p = max(0.0, min(100.0, p))
                odds[tf] = {"up": p, "down": 100.0 - p}
        return odds
    except Exception as e:
        print(f"Polymarket odds error {symbol}: {e}")
        return odds
...
def next_candle_consensus(data, technical_5m=None, technical_15m=None):
    """Eight-model next-candle consensus.

    This is an internal multi-model consensus, NOT a count of external
    websites. TradingView community predictors cannot be queried reliably
    through a public indicator-values API, so the bot calculates independent
    directional votes from the same Binance closed-candle data.
    """
    try:
        closed = data[:-1]
        if len(closed) < 80:
        ...
        # 2) EMA20 price position
        ema20 = _ema(c, 20)
        votes.append(1 if ema20 is not None and c[-1] > ema20 else
                     -1 if ema20 is not None and c[-1] < ema20 else 0)

        # 3) MACD direction
        mc = _macd(c)
        votes.append(1 if mc and mc[0] > mc[1] else
                     -1 if mc and mc[0] < mc[1] else 0)

        # 4) Stochastic
        st = _stoch(h, l, c, 14)
        votes.append(1 if st is not None and st < 20 else
                     -1 if st is not None and st > 80 else
                     1 if st is not None and st >= 50 else
                     -1 if st is not None else 0)

        # 5) CCI
        ci = _cci(h, l, c, 20)
        votes.append(1 if ci is not None and ci > 0 else
                     -1 if ci is not None and ci < 0 else 0)

        # 6) Williams %R
        hh = max(h[-14:]); ll = min(l[-14:])
        wr = -50 if hh == ll else -100 * (hh - c[-1]) / (hh - ll)
        votes.append(1 if wr < -50 else -1 if wr > -50 else 0)

        # 7) Awesome Oscillator
        ao = _ao(h, l)
        votes.append(1 if ao is not None and ao > 0 else
                     -1 if ao is not None and ao < 0 else 0)

        # 8) Technical Rating confirmation (5m + 15m)
        def rating_vote(t):
            if not t:
                return 0
            cat = t.get("category")
        ...
        r5 = rating_vote(technical_5m)
        r15 = rating_vote(technical_15m)
        if r5 == r15 and r5 != 0:
            votes.append(r5)
        elif r5 != 0 or r15 != 0:
            votes.append(r5 if r5 != 0 else r15)
        else:
            votes.append(0)
...
def _prob_from_technical(technical):
    if not technical:
        return None
    try:
        overall = float(technical.get("overall", 0.0))
    except (TypeError, ValueError):
        return None
    overall = max(-1.0, min(1.0, overall))
    return 50.0 + (overall * 50.0)


def _prob_from_consensus(consensus):
    if not consensus:
        return None
    try:
        bull = float(consensus.get("bullish", 0))
        bear = float(consensus.get("bearish", 0))
        total = float(consensus.get("total", 0))
    except (TypeError, ValueError):
        return None
    if total <= 0:
        return None
    ...
    return 50.0 + (net * 50.0)


def _prob_from_next_candle(next_candle):
    if not next_candle:
        return None
    direction = str(next_candle.get("direction", "Neutral"))
    try:
        conf = float(next_candle.get("confluence", 0))
        total = float(next_candle.get("total", 0))
    except (TypeError, ValueError):
        return None
    if total <= 0:
        return 50.0
    strength = max(0.0, min(1.0, conf / total))
    if direction == "Bullish":
        return 50.0 + (50.0 * strength)
    if direction == "Bearish":
        return 50.0 - (50.0 * strength)
    return 50.0


def _prob_from_rsi(rsi):
    try:
        rsi = float(rsi)
    except (TypeError, ValueError):
        return None
    # RSI is directional here: lower RSI -> higher bullish reversal bias;
    # higher RSI -> higher bearish pullback bias.
    p = 100.0 - rsi
    return max(5.0, min(95.0, p))


def _prob_from_trend_volume(trend, volume_ratio):
    direction = 1.0 if trend == "صعودی" else -1.0 if trend == "نزولی" else 0.0
    try:
        vr = float(volume_ratio)
    except (TypeError, ValueError):
        vr = 1.0
    # Higher relative volume increases the weight of the active trend;
    # it does not create a direction by itself.
    volume_strength = max(0.0, min(1.0, (vr - 0.5) / 2.5))
    strength = 0.35 + (0.65 * volume_strength)
    return 50.0 + (50.0 * direction * strength)


def calculate_next_candle_probability(
    rsi,
    volume_ratio,
    trend,
    market_probability,
    ...
    """Aggregate the factors already shown in the Telegram message.

    This is a heuristic model estimate, not a statistically calibrated
    probability. Duplicate Polymarket fields are intentionally counted once.
    """
    factors = []

    # Core market/technical factors.
    rsi_p = _prob_from_rsi(rsi)
    if rsi_p is not None:
        factors.append((rsi_p, 0.15))

    tv_p = _prob_from_technical(technical)
    if tv_p is not None:
        factors.append((tv_p, 0.20))

    consensus_p = _prob_from_consensus(consensus)
    if consensus_p is not None:
        factors.append((consensus_p, 0.20))

    next_p = _prob_from_next_candle(next_candle)
    if next_p is not None:
        factors.append((next_p, 0.10))

    # Trend + volume are combined so volume does not become a fake
    # independent direction signal.
    factors.append((_prob_from_trend_volume(trend, volume_ratio), 0.15))

    # market_probability and the displayed Polymarket UP/DOWN odds come
    # from the same market, so use this information only once.
    if market_probability is not None:
        try:
            mp = max(0.0, min(100.0, float(market_probability)))
            factors.append((mp, 0.20))
        except (TypeError, ValueError):
            pass
    ...
    bullish_probability = sum(prob * weight for prob, weight in factors) / total_weight
    bullish_probability = max(0.0, min(100.0, bullish_probability))
    bearish_probability = 100.0 - bullish_probability

    if bullish_probability >= 55.0:
        direction, emoji = "Bullish", "🟢"
    elif bullish_probability <= 45.0:
        direction, emoji = "Bearish", "🔴"
    else:
        direction, emoji = "Neutral", "🟡"
...
def build_message(symbol, timeframe, rsi, volume_ratio, volume_emoji, trend, trend_emoji, market_probability, polymarket_odds=None, technical=None, next_candle=None, consensus=None):
    # Each Telegram alert contains only the Polymarket odds for its own
    # signal timeframe (for example, a 5m alert shows only 5m odds).
    score = score_signal(rsi, volume_ratio, trend, market_probability)
    probability = calculate_next_candle_probability(
        rsi=rsi,
    ...
    # Show only the Polymarket timeframe that triggered this message.
    current_odds = (polymarket_odds or {}).get(timeframe)
    if current_odds and current_odds.get("up") is not None:
        polymarket_block = (
            f"{timeframe:<3}  UP {current_odds['up']:>5.1f}%  |  "
            f"DOWN {current_odds['down']:>5.1f}%"
        )
    else:
        polymarket_block = f"{timeframe:<3}  UP   N/A   |  DOWN   N/A"
    ...
    if consensus:
        ...
        neutral = consensus.get("neutral", 0)
        total = consensus.get("total", 8)
        consensus_icon = "🟢" if direction == "Bullish" else "🔴" if direction == "Bearish" else "🟡"
    else:
        direction = "N/A"
    ...
    if next_candle:
        nc_direction = next_candle.get("direction", "Neutral")
        nc_conf = next_candle.get("confluence", 0)
        nc_total = next_candle.get("total", 5)
        if nc_direction == "Bullish":
            nc_text = f"🟢 Bullish  {nc_conf}/{nc_total}"
        elif nc_direction == "Bearish":
            nc_text = f"🔴 Bearish  {nc_conf}/{nc_total}"
        else:
            nc_text = f"🟡 Neutral  {nc_conf}/{nc_total}"
    else:
        nc_text = "⚪ N/A"

    rsi_icon = "🔴" if rsi >= 70 else "🟢" if rsi <= 30 else "🟡"

    prob_bull = probability["bullish"]
    prob_bear = probability["bearish"]
    prob_direction = probability["direction"]
    prob_icon = probability["emoji"]

    return (
        f"<b>╔══════════════════════════════╗</b>\n"
        f"<b>║   ⚡ NEXT CANDLE RADAR      ║</b>\n"
        f"<b>╠══════════════════════════════╣</b>\n"
        f"<b>║  {symbol}  •  {timeframe:<3}             ║</b>\n"
        f"<b>╚══════════════════════════════╝</b>\n\n"

        f"<b>🎯 NEXT CANDLE CONSENSUS</b>\n"
        f"<code>┌──────────────────────────────┐\n"
        f"│  🟢 BULLISH     {bullish:>2}/{total:<2}           │\n"
        f"│  🔴 BEARISH     {bearish:>2}/{total:<2}           │\n"
        f"│  🟡 NEUTRAL     {neutral:>2}/{total:<2}           │\n"
        f"├──────────────────────────────┤\n"
        f"│  {consensus_icon} CONSENSUS  {direction:<11} │\n"
        f"└──────────────────────────────┘</code>\n\n"

        f"<b>🎲 POLYMARKET PROBABILITY</b>\n"
        f"<code>{polymarket_block}</code>\n\n"

        f"<b>📡 MARKET SNAPSHOT</b>\n"
        f"<code>"
        f"RSI             {rsi:>5.1f} {rsi_icon}\n"
        f"Volume          {volume_ratio:>5.1f}x {volume_emoji}\n"
        f"Technical       {technical_text}\n"
        f"Market          {market_text}\n"
        f"Next Candle     {nc_text}"
        f"</code>\n\n"

        f"<b>🧠 SIGNAL</b>\n"
        f"<code>🟢 Bullish   {prob_bull:>5.1f}%\n"
        f"🔴 Bearish   {prob_bear:>5.1f}%\n"
        f"{prob_icon} Prediction  {prob_direction}</code>\n"
        f"<i>Combined estimate from the factors above; not a guarantee.</i>\n\n"
        f"<b>━━━━━━━━━━━━━━━━━━━━━━━━━━━━</b>\n"
        f"🔗 <a href=\"{tv_link(symbol, timeframe)}\">Open TradingView</a>\n"
        f"<i>Closed-candle analysis • Multi-factor consensus</i>"
    )
...
def main():
    ...
    for signal in pending:
        ...
        polymarket_odds = {
            signal["timeframe"]: (
                {"up": probability, "down": 100.0 - probability}
                if probability is not None
                else None
            )
        }
        ...
        consensus = next_candle_consensus(next_data, tr5, tr15) if next_data else None
